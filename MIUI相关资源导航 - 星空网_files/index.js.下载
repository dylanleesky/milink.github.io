...
$(function() {
	...
	// ============================数据请求业务==============================
	$.getJSON(
		'https://sj.biada.cn/data/mydb.json',
		res => {
			// 页面渲染
	...
	$('#category').on('click', 'li', function() {
		...
		try {
			...
			try {
				var floorId = $(this).prev().attr('id');
				var floorName = floorId + 'Item';
				var floorTop = $('#' + floorName).offset().top + 300;
			} catch (e) {
				try {
					var floorId = $(this).next().attr('id');
					var floorName = floorId + 'Item';
					var floorTop = $('#' + floorName).offset().top - 300;
				} catch (e) {
					var floorId = $(this).prev().prev().attr('id');
					var floorName = floorId + 'Item';
					var floorTop = $('#' + floorName).offset().top + 600;
				}
			}
		}
